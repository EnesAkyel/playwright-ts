...
async function getHeapMb(page: import('@playwright/test').Page): Promise<number | null> {
    // performance.memory is a Chrome-only extension. Returns null in other browsers.
...
test.describe('Performance Metrics', { tag: ['@performance', '@regression'] }, () => {
    ...
    test('SauceDemo login page should load within budget', async ({ page, loginPage }) => {
        ...
        expect(navEntry.loadEventEnd, 'load event exceeded budget').toBeLessThan(BUDGET_MS.loadEvent);
    });

    test('SauceDemo inventory page should load within budget', async ({ page, loginPage, inventoryPage }) => {
        await epic('Performance');
        await feature('Page Load Times');
    ...
    test('JSONPlaceholder homepage should load within budget', async ({ page, homePage }) => {
        await epic('Performance');
        await feature('Page Load Times');
