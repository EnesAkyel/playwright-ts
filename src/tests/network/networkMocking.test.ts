...
const API_BASE = ENV.baseUrl; // https://jsonplaceholder.typicode.com

test.describe('Network Mocking / Request Interception', { tag: ['@network', '@regression'] }, () => {

    // ── 1. Mock a GET response with synthetic data ────────────────────────────
    ...
    // ── 2. Capture and assert POST request payload ────────────────────────────
    test('intercept POST /posts and verify request body payload', async ({ page }) => {
        ...
        await page.route(`${API_BASE}/posts`, async route => {
            if (route.request().method() === 'POST') {
                ...
                await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ ...expectedPayload, id: 101 }) });
            } else {
    ...
    // ── 5. Intercept and modify a real response ──────────────────────────────
    test('intercept /posts/1 and override the title in the real response', async ({ page }) => {
        await page.route(`${API_BASE}/posts/1`, async route => {
            ...
            await route.fulfill({ response, body: JSON.stringify({ ...body, title: 'Overridden Title' }) });
        });
    ...
    // ── 7. Block third-party analytics/tracking scripts ─────────────────────
    ...
    // ── 8. Add artificial latency to simulate a slow network ─────────────────
    ...
    // ── 9. Count how many times an endpoint is requested ────────────────────
    ...
    // ── 10. Wildcard pattern intercepts multiple related endpoints ────────────
